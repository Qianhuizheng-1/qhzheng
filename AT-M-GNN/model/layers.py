...
from model.attention import att_inter_agg, EnhancedMultiHeadAttention, SelfAttention
import numpy as np
import math
...
class InterAgg(nn.Module):
    ...
    def forward(
        ...
        seq_len = cat_feats.size(1) // self.feat_dim
        reshaped_feats = cat_feats.view(batch_size, seq_len, self.feat_dim)
        
        # 应用多头注意力
        attended_feats = self.attention(reshaped_feats)
        attended_feats = attended_feats.view(batch_size, -1)
        
        # 最终的特征变换
        combined = F.relu(attended_feats.mm(self.weight1).t())
...
class IntraAgg(nn.Module):
    def __init__(
        self, features, feat_dim, embed_dim, train_pos, rho, gen, rl, relation, cuda=False, dropout=0.1
    ):
    ...
    def forward(
        ...
        # 使用自注意力机制处理特征
        batch_size = self_feats.size(0)
        seq_len = 2  # self_feats 和 agg_feats
        combined_feats = torch.stack([self_feats, agg_feats], dim=1)  # [batch_size, 2, feat_dim]
        
        # 应用自注意力
        attended_feats = self.attention(combined_feats)
        attended_feats = attended_feats.view(batch_size, -1)

        # context_level contrast
        cat_feats = torch.cat((self_feats, agg_feats), dim=1)
        gen_feats, raw_feats = self.gen(cat_feats)
