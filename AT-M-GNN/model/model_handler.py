...
def find_suitable_num_heads(dimension):
    """
    Find a suitable number of attention heads that divides the dimension.
    Prioritize powers of 2.
    """
    if dimension % 8 == 0:
        return 8
    elif dimension % 4 == 0:
        ...
        # Fallback to 1 if no common power of 2 divisor found
        return 1
...
class ModelHandler(object):
    ...
    def train(self):
        ...
        if args.model == "DiG-In-GNN":
            ...
            suitable_num_heads = find_suitable_num_heads(feat_data.shape[1])
        ...
        if args.model == "DiG-In-GNN":
            if args.cuda:
                gnn_model = DiGInLayer(2, inter1, args.alpha, neigh_gen, torch.device("cuda:0"), adv_loss_weight=args.adv_loss_weight)
            else:
                gnn_model = DiGInLayer(2, inter1, args.alpha, neigh_gen, torch.device("cpu"), adv_loss_weight=args.adv_loss_weight)
        ...
        for epoch in range(args.num_epochs):
            ...
            for batch in range(num_batches):
                ...
                # 判断当前epoch是否进行对抗训练
                is_adversarial_epoch = False
                if args.adversarial_training and epoch >= args.rl_update_start:
                    # 计算当前epoch应该进行哪种训练
                    epoch_in_cycle = (epoch - args.rl_update_start) % args.alternate_frequency
                    is_adversarial_epoch = epoch_in_cycle >= (args.alternate_frequency // 2)

                if is_adversarial_epoch:  # 进行对抗训练
                    # 动态调整对抗扰动的强度
                    current_epsilon = args.epsilon * min(1.0, (epoch - args.rl_update_start) / args.adv_warmup_epochs)
                    loss, ctx_, env_ = gnn_model.adversarial_training(batch_nodes, labels, optimizer, epsilon=current_epsilon)
                else:  # 进行普通训练
            ...
            if epoch % args.valid_epochs == 0 and epoch >= args.gnn_update_start:
                ...
                if args.model == "SAGE" or args.model == "GCN":
                    ...
                    # 获取多指标优化配置
                    multi_metrics = getattr(args, 'multi_metrics', None)
                    optimize_threshold = getattr(args, 'optimize_threshold', True) if hasattr(args, 'optimize_threshold') else args.get('optimize_threshold', True)
                    f1_mac_val, f1_1_val, f1_0_val, auc_val, gmean_val = test_dig(
                        ...
                        optimize_metric=getattr(args, 'optimize_metric', 'f1_macro'),
